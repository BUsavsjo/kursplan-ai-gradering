export const AIAS_MUS = {
  FORBJUDET: {
    icon: "⛔",
    words: [
      "enkla", "enkel", "enkelt",
      "återge", "härma", "imitera",
      "känna igen", "gissa",
      "klappa efter", "sjunga med",
      "spela efter", "följ med",
      "i huvudsak", "delvis", "någon mån",
      "översiktligt", "grundläggande",
      "namnge", "lista", "memorera",
      "enstaka toner", "enkelt komp",
      "bara puls", "bara takt",
      "utan begrepp", "utan motivering",
      "i huvudsak anpassa", "viss säkerhet",
      "något exempel"
    ]
  },
  INTRODUCERA: {
    icon: "🌱",
    words: [
      "beskriva", "resonera", "jämföra", "förklara",
      "använda musikbegrepp", "använder musikbegrepp",
      "puls", "rytm", "takt", "tempo", "dynamik", "klang", "melodi", "harmonik", "form",
      "fraser", "perioder", "taktarter",
      "vers", "refräng", "brygga", "intro", "outro",
      "sjunga unison", "sjunger unison", "kanon",
      "spela stämma", "spela melodi", "kompa på ackord",
      "ackordföljd", "tonika", "dominant",
      "samspel", "delta i ensemble", "följa dirigering",
      "notläsning enkel", "tabulatur",
      "lyssna aktivt", "genre",
      "använda digitalt verktyg", "loopar", "MIDI", "inspelning enkel",
      "relativt god säkerhet", "anpassa väl", "exempel"
    ]
  },
  BEARBETA: {
    icon: "✏️",
    words: ["bearbeta","redigera","förtydliga","omformulera","förbättra"]
  },
  FORVANTAT: {
    icon: "📌",
    words: [
      "dra slutsatser", "analysera", "värdera", "diskutera", "reflektera",
      "tolka", "motivera val", "ståndpunkter och argument",
      "gestalta", "improvisera", "komponera", "arrangera",
      "planera repetition", "sätta mål", "strategi för övning",
      "flerstämmighet", "stämsång", "ackompanjera",
      "genretypiska drag", "stilkännetecken", "epok",
      "notera egen musik", "gehör och notation",
      "ljudproduktion", "mixning grund", "mikrofonteknik",
      "upphovsrätt", "källkritik i musik", "licenser",
      "scenisk närvaro", "kommunicera med publik",
      "dokumentera process", "utvärdera arbetet",
      "god säkerhet", "anpassa"
    ]
  },
  INTEGRERAT: {
    icon: "🔗",
    words: [
      "välutvecklade resonemang", "välgrundade", "nyanserat", "problematisera",
      "källkritiska argument", "relevans och trovärdighet",
      "gestaltningsdjup", "rolltolkning", "interpretation",
      "för processen framåt", "ledarskap i ensemble", "dirigera",
      "arrangera för besättning", "orkestrera", "förfina", "fördjupa",
      "avancerad improvisation", "tematisk utveckling", "variation och kontrast",
      "harmonisk analys", "formanalys", "polyrtmik",
      "produktion och mix", "mastering översiktlig", "automation i DAW",
      "intertextuella referenser", "kulturella perspektiv",
      "konstruktiv återkoppling", "utvärdera med kriterier",
      "väl fungerande samspel", "anpassa mycket väl", "mycket god säkerhet"
    ]
  }
};
